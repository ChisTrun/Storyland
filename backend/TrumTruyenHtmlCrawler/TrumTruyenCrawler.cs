	...
	public List<Story> GetStoriesBySearchName(string storyName)
	{
		...
		try
		{
			...
			for (var i = 1; i <= lastIndex; i++)
			{
				...
				tasks.Add(Task.Run(() =>
				{
					var nodeTasks = new List<Task<Story>>();
					...
					foreach (var node in curPage.QuerySelectorAll("div[itemtype=\"https://schema.org/Book\"]"))
					{
						...
						{
							var imageLink = nodeCopy.QuerySelectorAll("img").Count() != 0
	...
	public List<Story> GetStoriesOfAuthor(string authorId)
	{
		try
		{
			...
			for (var i = 1; i <= lastIndex; i++)
			{
				...
				{
					var nodeTasks = new List<Task<Story>>();
					var curPage = LoadHtmlDocument($"{HOME_URL}/tac-gia/{authorId}/trang-{iCopy}");
					foreach (var node in curPage.QuerySelectorAll("div[itemtype=\"https://schema.org/Book\"]"))
					{
						var nodeCopy = node;
						nodeTasks.Add(Task.Run(() =>
	...
	public List<Story> GetStoriesOfCategory(string categoryId)
	{
		try
		{
			...
			for (var i = 1; i <= lastIndex; i++)
			{
				...
				{
					var nodeTasks = new List<Task<Story>>();
					var curPage = LoadHtmlDocument($"{HOME_URL}/the-loai/{categoryId}/trang-{iCopy}");
					foreach (var node in curPage.QuerySelectorAll("div[itemtype=\"https://schema.org/Book\"]"))
					{
						var nodeCopy = node;
						nodeTasks.Add(Task.Run(() =>
