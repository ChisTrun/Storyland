...
namespace TrumTruyenHtmlCrawler
{
    public class TrumTruyenCrawler : ICrawler
    {
        ...
        public string Description => "Trùm truyện";
        ...
        public IEnumerable<Chapter> GetChaptersOfStory(string storyId)
        {
            ...
            if (totalPage == 1)
            {
                ...
                {
                    chapters.Add(new Chapter(node.InnerText, storyId, story, int.Parse(_regex03.Match(node.GetAttributeValue("href", "")).Groups[1].Value) - 1));
                }
                return chapters;
            }
        ...
        public PagingRepresentative<Chapter> GetChaptersOfStory(string storyId, int page, int limit)
        {
            ...
            while (count < limit)
            {
                ...
                for (; startIndex < DEFAULT_PAGE_SIZE; startIndex++)
                {
                    ...
                    chapters.Add(new Chapter(nodes[startIndex].InnerText, storyId, story, int.Parse(_regex03.Match(nodes[startIndex].GetAttributeValue("href", "")).Groups[1].Value) - 1));
                }
                startIndex = 0;
                startPage += 1;
            }
        ...
        public PagingRepresentative<Story> GetStoriesBySearchName(string storyName, int page, int limit)
        {
            ...
            while (count < limit)
            {
                ...
                {
                    count++;
                    if (count > limit || count > totalStory || startIndex >= nodes.Count()) break;
