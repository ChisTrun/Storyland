...
public partial class TangThuVienCrawler : ICrawler
{
	...
	public List<Story> GetStoriesBySearchName(string storyName)
	{
		...
		var liAbleNextPageSelector = "body > div.rank-box.box-center.cf > div.main-content-wrap.fl > div.page-box.cf > div > ul > li > a";
		var doc = GetWebPageDocument(createUrlFromPage.Invoke(1));
		var aTags = doc.QuerySelectorAll(liAbleNextPageSelector);
		var pages = aTags.Where(a => Regex.IsMatch(a.GetDirectInnerTextDecoded(), @"^\d+$")).Select(a => int.Parse(a.GetDirectInnerTextDecoded()));
		var lastPage = !pages.Any() ? 1 : pages.Max();
		var stories = GetRepresentativesFromATagsFromAllPages(RankViewListFormat.CrawlStoriesFromAPage, createUrlFromPage.Invoke, lastPage);
		return stories;
	}
	...
	public List<Story> GetStoriesOfAuthor(string authorId)
	{
		...
		var liAbleNextPageSelector = "body > div.rank-box.box-center.cf > div.main-content-wrap.fl > div.page-box.cf > div > ul > li > a";
		var doc = GetWebPageDocument(createUrlFromPage.Invoke(1));
		var lastPage = doc.QuerySelectorAll(liAbleNextPageSelector).Where(a => Regex.IsMatch(a.GetDirectInnerTextDecoded(), @"^\d+$")).Select(a => int.Parse(a.GetDirectInnerTextDecoded())).Max();
		var stories = GetRepresentativesFromATagsFromAllPages(RankViewListFormat.CrawlStoriesFromAPage, createUrlFromPage.Invoke, lastPage);
		return stories;
	}
	...
	public ChapterContent GetChapterContent(string storyId, int index)
	{
		...
		GetNameContentStoryOfChapter(chapterDoc, out var chapterName, out var content);
		//PrevNextChapId(chaptersDoc, currentChapterIndex, out var prevChapId, out var nextChapId);
		return new ChapterContent(content, chapterName, currentChapterIndex - 1, storyId);
	}
	...
	private static List<T> GetRepresentativesFromATagsFromAllPages<T>(Func<HtmlDocument, List<T>> crawlRepresentativesFromAPage, Func<int, string> createURLFromPage, int lastPage) where T : class
	{
		var page = 1;
		...
		while (page <= lastPage)
		{
			...
			tasks.Add(Task.Run(() =>
			{
				...
				try
				{
					document = GetWebPageDocument(accessUrl);
					results = crawlRepresentativesFromAPage.Invoke(document);
				}
				catch (CrawlerDocumentException)
				{
					results = new List<T>();
				}
				return results;
	...
	private void GetNameContentStoryOfChapter(HtmlDocument chapterDoc, out string chapterName, out string content)
	{
